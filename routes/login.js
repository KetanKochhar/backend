...
router.post("/login",async (request,response)=>{
    ...
    if (user){
        ...
        if (isPasswordMatch){
            ...
            response.render("login", {error: "Wrong E-mail or password"});
        }
    }
    else{
        response.render("login", {error: "i think this account is not registered "});
    }
})
...
router.get("/otp",(request,response)=>{
    if (request.session.tempuser){
        ...
        return response.render("otp" , {error:null,mail:request.session.mail});
    }
    response.redirect("/");
})
...
router.post("/otp",async(request,response)=>{
    ...
    if(res.otp_code == otp.join("")){
        ...
        response.render("otp",{error: true , mail:request.session.mail})
    }
})

router.get("/forgot-password",async(request,response)=>{
    response.render("password.ejs",{error:null})
})
...
router.post("/forgot-password", async (request, response) => {
    ...
    try {
        ...
        if (!user) {
            return response.render("password", { error: "Email not found" });
        }
        ...
        return response.render("password", { error: "Something went wrong, try again later." });
    }
});
...
router.get("/reset-password", (request, response) => {
    ...
    response.render("reset-password", { error: null });
});
...
router.post("/reset-password", async (request, response) => {
    ...
    try {
        ...
        if (savedOtp.otp_code !== otp) {
            return response.render("reset-password", { error: "Invalid OTP" });
        }
        ...
        response.render("login",{error:"passwords updated successfully please login again"});
    } catch (error) {
        console.error("Reset password error:", error.message);
        response.render("reset-password", { error: "Failed to reset password. Try again." });
    }
});

...
document.getElementById('addressForm').addEventListener('submit', async (e) => {
  ...
  const isUpdating = document.getElementById('openUpdateBtn') !== null;
  const endpoint = isUpdating ? '/api/update-address' : '/api/address';
...
function showToast(message, type = 'success') {
  ...
  toast.className = `fixed bottom-4 right-4 p-4 rounded-lg shadow-lg text-white text-sm transition-all duration-300 transform translate-y-0 opacity-100 z-50 ${
...
async function updateButtonText() {
  ...
  try {
      ...
      document.querySelectorAll('.bg-blue-600.text-white.px-6.py-3.rounded-lg').forEach(button => {
          ...
          button.textContent = `Add to Cart${quantity > 0 ? ` (${quantity})` : ''}`;
      });
  } catch (error) {
